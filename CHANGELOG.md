## [1.16.0] - 2026-06-26
### Added
* レイアウトを複数保存・エクスポートする機能 (#395)

## [1.15.0] - 2025-10-10
### Changed
* Funcの引数と戻り値の配列型対応 (#392)
* LogCardのスクロール表示を改善、UI変更 (#393)
* LogCardに表示するログメッセージのスペースを保持 (#393)

## [1.14.0] - 2025-02-08
### Added
* Gamepad API の値を読み送信する機能を追加 (#366)
### Fixed
* SideMenuのボタン名が長いとき自動的に改行されるのを防止 (#367)
* ValueCardの挙動を修正 (#368)
    * ときどきValueCardの表示が更新されなくなる場合があるのを修正
    * 値の表示位置を修正、配列の場合そのすべての要素で小数点の位置がそろうようにした

## [1.13.0] - 2025-01-30
### Added
* Funcを登録順(index順)に並べ替える機能を追加 (#355)
* 配列型Valueの表示機能 (#356)
### Fixed
* funcCardを開くとCPUを100%消費するバグを修正 (#355)
* 数値の表示桁数や指数表示の処理を改良 (#356)
* グラフの値が更新されたときマウスカーソルを動かさなくてもカーソル位置の値の表示を更新するようにした (#356)
